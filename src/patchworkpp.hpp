...
bool point_z_cmp(PointTP a, PointTP b) { return a.z < b.z; }
...
{   
    ...
    RevertCandidate(int _c_idx, int _s_idx, double _flatness, double _line_var, Eigen::Vector4f _pc_mean, pcl::PointCloud<PointTP> _ground)
...
private:
    ...
    void pc2czm(const pcl::PointCloud<PointTP> &src, std::vector<Zone> &czm, pcl::PointCloud<PointTP> &cloud_nonground);

    void reflected_noise_removal(pcl::PointCloud<PointTP> &cloud, pcl::PointCloud<PointTP> &cloud_nonground);
    
    void temporal_ground_revert(pcl::PointCloud<PointTP> &cloud_ground, pcl::PointCloud<PointTP> &cloud_nonground,
    ...
    void extract_piecewiseground(
            const int zone_idx, const pcl::PointCloud<PointTP> &src,
            pcl::PointCloud<PointTP> &dst,
            pcl::PointCloud<PointTP> &non_ground_dst);

    void extract_initial_seeds(
            const int zone_idx, const pcl::PointCloud<PointTP> &p_sorted,
...
template<typename PointTP> inline
void PatchWorkpp<PointTP>::initialize_zone(Zone &z, int num_sectors, int num_rings) {
    z.clear();
...
template<typename PointTP> inline
void PatchWorkpp<PointTP>::flush_patches_in_zone(Zone &patches, int num_sectors, int num_rings) {
    for (int i = 0; i < num_sectors; i++) {
        for (int j = 0; j < num_rings; j++) {
...
template<typename PointTP> inline
void PatchWorkpp<PointTP>::flush_patches(vector<Zone> &czm) {
...
template<typename PointTP> inline
void PatchWorkpp<PointTP>::estimate_plane(const pcl::PointCloud<PointTP> &ground) {
...
template<typename PointTP> inline
void PatchWorkpp<PointTP>::extract_initial_seeds(
        const int zone_idx, const pcl::PointCloud<PointTP> &p_sorted,
        pcl::PointCloud<PointTP> &init_seeds, double th_seed) {
...
template<typename PointTP> inline
void PatchWorkpp<PointTP>::reflected_noise_removal(pcl::PointCloud<PointTP> &cloud_in, pcl::PointCloud<PointTP> &cloud_nonground)
{
...
template<typename PointTP> inline
void PatchWorkpp<PointTP>::estimate_ground(
        ...
        for (int ring_idx = 0; ring_idx < num_rings_each_zone_[zone_idx]; ++ring_idx) {
            for (int sector_idx = 0; sector_idx < num_sectors_each_zone_[zone_idx]; ++sector_idx) {
                ...
                */
                ...
                {
                    RevertCandidate<PointTP> candidate(concentric_idx, sector_idx, ground_flatness, line_variable, pc_mean_, regionwise_ground_);
...
template<typename PointTP> inline
void PatchWorkpp<PointTP>::update_elevation_thr(void)
{
...
template<typename PointTP> inline
void PatchWorkpp<PointTP>::update_flatness_thr(void)
{
    for (int i=0; i<num_rings_of_interest_; i++)
    {
...
template<typename PointTP> inline
void PatchWorkpp<PointTP>::temporal_ground_revert(pcl::PointCloud<PointTP> &cloud_ground, pcl::PointCloud<PointTP> &cloud_nonground,
                                               std::vector<double> ring_flatness, std::vector<RevertCandidate<PointTP>> candidates,
                                               int concentric_idx)
{
...
// For adaptive
template<typename PointTP> inline
void PatchWorkpp<PointTP>::extract_piecewiseground(
        const int zone_idx, const pcl::PointCloud<PointTP> &src,
        pcl::PointCloud<PointTP> &dst,
        pcl::PointCloud<PointTP> &non_ground_dst) {
...
template<typename PointTP> inline
void PatchWorkpp<PointTP>::calc_mean_stdev(std::vector<double> vec, double &mean, double &stdev)
{
...
template<typename PointTP> inline
double PatchWorkpp<PointTP>::xy2theta(const double &x, const double &y) { // 0 ~ 2 * PI
    // if (y >= 0) {
...
template<typename PointTP> inline
double PatchWorkpp<PointTP>::xy2radius(const double &x, const double &y) {
    return sqrt(pow(x, 2) + pow(y, 2));
}

template<typename PointTP> inline
void PatchWorkpp<PointTP>::pc2czm(const pcl::PointCloud<PointTP> &src, std::vector<Zone> &czm, pcl::PointCloud<PointTP> &cloud_nonground) {
